...
| 2 | VA All Employee Survey | **Built 2026-09-12** for 2018, 2020, 2022 and 2023, at VA and its three administrations, including an engagement index constructed the way OPM constructs theirs. The published form is a PDF of response distributions carrying full FEVS item wording, so items are matched by text; values are rounded to whole percents and notated `estimate`. Not built: 2024, which exists only as a tabular file keyed by unpublished item numbers (`f03`, `f04`, …); and a comparable satisfaction index, because VA does not ask the pay item. |
