...
1. **Measures over time.** 96 measures, 12.2 million values, 984 nodes, January 2005 to July 2026 monthly, with money by fiscal quarter and surveys by survey year.
...
| Object | Where | Notes |
|---|---|---|
| Measure | `catalog/measures.json` | 96 entries: code, definition, unit, aggregation, extensiveness, category, cadence, since-date, levels, dimensions, sources, calculation |
...
| # | Item | State |
|---|---|---|
| 1 | OSHA injury and illness rates | **Built 2026-09-12**, FY2004 to FY2019, 27 agencies, through a validation gate that recomputes each published rate from its own case count and employment. OSHA has published nothing since FY2019, so the measures carry an `until`. |
| 2 | EEOC complaint rates | **Attempted and not shipped.** Per-agency statistics live inside annual report PDFs; the profiles page and the Form 462 resources page carry no tables and no downloadable dataset, so there is no artefact to put through the gate. |
| 3 | VA All Employee Survey | **Built**, 2018, 2020, 2022 and 2023, at VA and its three administrations, with an engagement index built the way OPM builds theirs. 2024 exists only as a tabular file keyed by unpublished item numbers. VA does not ask the pay item, so there is no comparable satisfaction index. |
| 4 | FEVS below agency level | **Built for 2019**, the only public year with a sub-agency field: 117 of 238 units, matched by name and never by code. 2020 through 2023 have no such field and the 2024 file is corrupt as published. |
| 5 | Historical org names | **Resolved.** All 165 units that rendered as their own code are named, harvested from 13 historical employment files and committed as reference data. |
| 6 | Cross-source reconciliation | **Built.** `docs/RECONCILIATION.md`, regenerated by `cli reconcile`. Eleven small-agency spending flags remain without a stated cause. |
| 7 | Repo location | In iCloud Drive by explicit decision. The sweep runs before every pipeline command and before the test suite; it mitigates and does not fix. |
| 8 | Product surface | No sharing, no saved views, no alerting. Deliberate. |

### The failure mode worth remembering

A source the build reads that CI cannot see is dropped silently, and a green deploy is not evidence against it. This has now happened three times: PLUM, the VA survey, and 2019 sub-agency FEVS. Every derived source is therefore committed as a reference table, and the check that catches it is comparing the CI build's row count against the local one.
