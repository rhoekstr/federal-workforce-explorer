...
- **Set `FEDPULSE_DATA=~/.fedpulse` for every pipeline command.** The repo lives in iCloud Drive (Desktop); raw and work files there get evicted and re-fetched, which made a ten-second extraction take seven minutes. With the variable set, `data/raw` and `data/work` resolve to `~/.fedpulse/raw` and `~/.fedpulse/work`, outside sync. The workflow on GitHub runners does not need it.
