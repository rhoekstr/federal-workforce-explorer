...
- **A data-loss incident, and the guard it produced.** `build_measures` assigned `manifest["measures"]` wholesale, so every local rebuild dropped the `release_url` and `extracts_url` that `publish_measures` had written. CI then found no extract archive to restore, built from the 57 months it had locally, and published a 2018-onward table over the 21-year one. The published data was wrong for about forty minutes. Three changes: the manifest entry is merged rather than replaced; the build refuses to run when it sees fewer than 80% of the extracts the last published build used; and restore logs why it skipped instead of returning silently. The guard is the one that matters, because it fails the run rather than shipping a smaller truth.
